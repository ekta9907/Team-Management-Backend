...
module.exports = {
  //================================================dashboard============================================================
  async getUnitsCount(deleteFlag) {
    try {
      const getCountDocuments = await Unit.countDocuments({ deleteFlag: deleteFlag });
  ...
  async getShiftsCount(deleteFlag) {
    try {
      const getCountDocuments = await Shift.countDocuments({ deleteFlag: deleteFlag });
      if (getCountDocuments) {
        return getCountDocuments;
      } else {
        return 0;
  ...
  async getUserCount(deleteFlag) {
    try {
      const getCountDocuments = await User.countDocuments({ deleteFlag: deleteFlag, roleName: { $ne: "Super-Admin" } });
      if (getCountDocuments) {
        return getCountDocuments;
      } else {
        return 0;
  ...
  async getEmployeeCounts(deleteFlag) {
    try {
      const getCountDocuments = await User.countDocuments({ deleteFlag: deleteFlag, roleName: { $nin: ["Super-Admin", "Admin"] } });
      if (getCountDocuments) {
        return getCountDocuments;
      } else {
        return 0;
  ...
  async getAdminCount(deleteFlag) {
    try {
      const getCountDocuments = await User.countDocuments({ deleteFlag: deleteFlag, roleName: { $eq: "Admin" } });
      if (getCountDocuments) {
        return getCountDocuments;
      } else {
        return 0;
  ...
  async getEmployeeCount(relievingStatus, deleteFlag) {
    try {
      const getCountDocuments = await User.countDocuments({ deleteFlag: deleteFlag, relievingStatus: relievingStatus, roleName: { $ne: "Super-Admin" } });
      if (getCountDocuments) {
        return getCountDocuments;
      } else {
        return 0;
  ...
  async checkEmployeeOne(employeeId) {
    try {
      const shiftStatus = await User.findOne({ _id: employeeId }); // 20 seconds timeout
  ...
  async getEmployees(deleteFlag) {
    try {
      const user = await User.aggregate([
        {
          $match: { deleteFlag: deleteFlag, approveFlag: 1, roleName: { $ne: "Super-Admin" } },
        },
        {
          $lookup: {
            from: "roles",
  ...
  async editPermissionAll(roleName, accessLevel) {
    try {
      const updateStatus = await User.updateMany({ roleName: roleName }, { $set: { accessLevel: accessLevel } });
      if (updateStatus) {
        return updateStatus.modifiedCount;
      } else {
        return 0;
  ...
  async updateEmployeeFields(userId, updateEmployeeFields) {
    try {
      const updateStatus = await User.updateOne({ _id: userId }, { $set: updateEmployeeFields });
      if (updateStatus) {
        return updateStatus.modifiedCount;
      } else {
        return 0;
      ...
      console.log("database error from admin service updateEmployeeFields", error.message);
      throw new Error(error.message);
    }
  },
  ...
  async getEmployeeByUniqueId(uniqueId) {
    try {
      const getEmployee = await User.findOne({ deleteFlag: 0, uniqueId: uniqueId });
      if (getEmployee) {
        return getEmployee;
      } else {
        return "NA";
      ...
      console.log("database error from admin service getEmployee", error.message);
      throw new Error(error.message);
    }
  },
  ...
  async getTeamByTeamNameWithUnitId(teamName, unitId) {
    try {
      const getTeams = await Team.findOne({ deleteFlag: 0, teamName: teamName, unitId: unitId });
      if (getTeams) {
        return getTeams;
      } else {
        return "NA";
