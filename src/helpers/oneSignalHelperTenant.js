...
const UserNotificationModel = require("../models/workspaceModels/userNotificationModel");
...
const oneSignalAuthorization =
  "os_v2_app_67pqocen3bag7aj4xzwr3rdbroygqyy52qxurs45peoqoujxgcvc6r6kpjtsmjzwxaswxjijpq33elxtzwr7nbnn2f7jypkyiar2fti";
const driverOneSignalAppId = "8903856a-b219-4665-9a75-8b356bd58ce6";
const driverOneSignalAuthorization =
  "ZTAwMTFjZTktYzk3ZS00ZjBjLWI4ZTMtMjg4NDBkMjAxNWFm";

module.exports = {
  async DeviceTokenStore_1_Signal(
    SITE_DB_NAME,
    userId,
    deviceType,
    loginType,
    ...
    try {
      const UserNotification = await UserNotificationModel(SITE_DB_NAME);
  ...
  async checkUserActiveDeactive(SITE_DB_NAME, userId) {
    try {
  ...
  async getUserPlayerId(SITE_DB_NAME, userId) {
    try {
      const UserNotification = await UserNotificationModel(SITE_DB_NAME);
  ...
  async getUserDeviceType(SITE_DB_NAME, userId, playerId) {
    try {
      const UserNotification = await UserNotificationModel(SITE_DB_NAME);
  ...
  async getUserLanguageId(SITE_DB_NAME, userId) {
    try {
      const User = await UserModel(SITE_DB_NAME);
  ...
  async getUserLoginType(SITE_DB_NAME, userId, playerId) {
    try {
      const UserNotification = await UserNotificationModel(SITE_DB_NAME);
      const result = await UserNotification.findOne({
        userId,
  ...
  async getNotificationStatus(SITE_DB_NAME, userId) {
    try {
      const User = await UserModel(SITE_DB_NAME);
      const result = await User.findOne({
        _id: userId,
  ...
  async getNotificationArrSingle(
    ...
    try {
      ...
      const [languageId, playerId] = await Promise.all([
        this.getUserLanguageId(SITE_DB_NAME, otherUserId),
        this.getUserPlayerId(SITE_DB_NAME, otherUserId),
      ]);
      if (!playerId || playerId === "no") {
        return "NA";
      }
      const loginType = await this.getUserLoginType(
        SITE_DB_NAME,
        otherUserId,
        playerId
      );
      const deviceType = await this.getUserDeviceType(
        SITE_DB_NAME,
        otherUserId,
        playerId
      );
  ...
  async getNotificationMsgCountOtherUser(SITE_DB_NAME, userId) {
    try {
      const UserNotificationMessage = await UserNotificationMessageModel(
        SITE_DB_NAME
      );
  ...
  async getNotificationMsgCount(SITE_DB_NAME, userId) {
    try {
      const UserNotificationMessage = await UserNotificationMessageModel(
        SITE_DB_NAME
      );
